...
export const FilterStationaryItems = () => {
  ...
  const clickHandler = () => {
    const newItems = items.filter(({price}) => price < 100);
    setItems(newItems);
  }
  ...
  return <div className="bg-gray-200 text-black p-4 rounded-lg shadow-md">
      <h2 className="text-xl font-semibold mb-4">Products</h2>
      {
        items.length !==0 ? <>
          ...
          <button onClick={clickHandler} className="text-white mt-2">Filter by Price</button>
        </> : ""
      }
  </div>
}