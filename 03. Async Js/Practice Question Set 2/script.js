...
// const result = fakeFetch("https://example.com/post/comments").then(res => {
...
// asyncResult("https://example.com/post/comments");
...
//     const winners = result.data.data.join(", ");
//     outputDiv.innerText = `Congratulation to the members of winning team ${winners} great work keep it up.`
//   } catch(err) {
//     if(err.status === 404) {
//       outputDiv.innerText = err.message;
//     }
...
// asyncResult("https://example.com/winner-team");
...
//     outputDiv.innerText = result.data.auth ? "Verified" : "Not Verified";
//     }catch(err) {
//       if(err.status === 404) {
//         outputDiv.innerText = err.message;
//       } else {
//         outputDiv.innerText = `Error from server: ${err}`
//       }
//     }
//   }

// asyncResult("https://example.com/login");
...
//     outputDiv.innerText = `Hello ${result.data.order.userName} your order status is ${result.data.order.status}.`
//   } catch(err) {
//     if (err.status === 404) {
//       outputDiv.innerText = err.message;
//     } else {
...
// asyncResult("https://example.com/order/status/OR00A12");
...
// asyncResult("https://example.com/photo");
...
//     const result = await fakeFetch(url);
//     console.log(result.data);
//     const totalPrice = result.data.reduce(((accumulator,{price,quantity}) => accumulator + (price * quantity)),0)
//     outputDiv.innerText = `Total: INR ${totalPrice}`;
//   } catch (err) {
//     if(err.status === 404) {
//       outputDiv.innerText = err.message;
//     }
...
// asyncResult("https://example.com/api/productlist");
...
//   } catch(err) {
//     if(err.status === 500) {
//       outputDiv.innerText = "Oops. Unexpected Error. Please try again.";
//       outputDiv.style.color = "red";
//     }
//   }
// }

// asyncResult("https://example.com/api/users");
...
//   } catch(err) {
//     if(err.status === 400) {
//       outputDiv.innerText = `${err.message}! Requested size too large. `
//     }
//   }
// }

// asyncResult("https://example.com/api/allbooks");
// Your Code here
...
//   try{
//     const result = await fakeFetch(url);
//     console.log(result.data.logged);
//     if(result.data.logged) {
//       outputDiv.innerText = result.data?.message ?? "Welcome to the server";
//     }
//   } catch (err) {
//     outputDiv.innerText = err.message;
//   }
...
// asyncResult("https://example.com/welcome");
// your code here
...
// const asyncResult = async (url,dimensions) => {
//   console.log(fakeFetch(url,dimensions));
//   try {
//     const result = await fakeFetch(url,dimensions);
//     const displayImage = document.createElement("img");
//     outputDiv.append(displayImage);
//     displayImage.src = result.data.image.link;
//     displayImage.alt = result.data.image.title;
//   } catch (err) {
//     if(err.status === 404) {
//       outputDiv.innerText = err.message;
//     } else {
//       outputDiv.innerText = `Error from server: ${msg}`;
//     }
//   }
// }

// asyncResult("https://example.com/getImage",[500,300]);
